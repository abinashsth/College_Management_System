...
<div class="container mx-auto px-4 py-6">
    <div class="flex flex-col md:flex-row items-start mb-6">
        <div class="flex-1">
            <h1 class="text-2xl font-bold text-gray-900">Create Schedule for {{ $exam->title }}</h1>
            <p class="text-gray-600">Create a new schedule for this exam</p>
        </div>
    ...
    <div class="bg-white rounded-lg shadow-md overflow-hidden">
        ...
        <form action="{{ route('exams.store-schedule', $exam) }}" method="POST" class="p-6">
            @csrf
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="date" id="exam_date" name="exam_date" value="{{ old('exam_date', $exam->exam_date ? $exam->exam_date->format('Y-m-d') : '') }}" 
                ...
                <div>
                    ...
                    <input type="time" id="start_time" name="start_time" value="{{ old('start_time', $exam->start_time ? date('H:i', strtotime($exam->start_time)) : '') }}" 
                ...
                <div>
                    ...
                    <input type="time" id="end_time" name="end_time" value="{{ old('end_time', $exam->end_time ? date('H:i', strtotime($exam->end_time)) : '') }}" 
                ...
                <div>
                    ...
                    <input type="text" id="location" name="location" value="{{ old('location', $exam->location ?? '') }}" 
                ...
                <div>
                    ...
                    <input type="text" id="room_number" name="room_number" value="{{ old('room_number', $exam->room_number ?? '') }}" 
                ...
                <div>
                    ...
                    <input type="number" id="seating_capacity" name="seating_capacity" min="1" value="{{ old('seating_capacity') }}" 
                ...
                <div>
                    ...
                    <select id="status" name="status" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-[#37a2bc] focus:ring focus:ring-[#37a2bc] focus:ring-opacity-50 @error('status') border-red-300 @enderror" required>
                        ...
                        <option value="{{ $value }}" @if(old('status') == $value || (!old('status') && $value == 'scheduled')) selected @endif>
                            {{ $label }}
                        </option>
                        @endforeach
                    </select>
                ...
                <div>
                    <div class="flex items-center">
                        <input type="checkbox" id="is_rescheduled" name="is_rescheduled" value="1" @if(old('is_rescheduled')) checked @endif
                ...
                <div id="reschedule_reason_container" class="@if(!old('is_rescheduled')) hidden @endif col-span-1 md:col-span-2">
                    ...
                    <textarea id="reschedule_reason" name="reschedule_reason" rows="2" 
                        class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-[#37a2bc] focus:ring focus:ring-[#37a2bc] focus:ring-opacity-50 @error('reschedule_reason') border-red-300 @enderror">{{ old('reschedule_reason') }}</textarea>
                ...
                <div class="col-span-1 md:col-span-2">
                    ...
                    <textarea id="notes" name="notes" rows="3" 
                        class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-[#37a2bc] focus:ring focus:ring-[#37a2bc] focus:ring-opacity-50 @error('notes') border-red-300 @enderror">{{ old('notes') }}</textarea>
                    @error('notes')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
