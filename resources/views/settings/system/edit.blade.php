...
@section('content')
    <div class="container mx-auto px-4 py-8">
        <div class="flex justify-between items-center mb-6">
            <h1 class="text-2xl font-bold text-gray-800">Edit System Setting</h1>
        ...
        <div class="bg-white shadow overflow-hidden rounded-lg">
            <form action="{{ route('settings.system.update', $setting->id) }}" method="POST" class="p-6">
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="col-span-1">
                        <label for="group_select" class="block text-sm font-medium text-gray-700 mb-1">Group <span class="text-red-600">*</span></label>
                        <div class="flex gap-2">
                            <select name="" id="group_select" 
                                    class="mt-1 focus:ring-blue-500 focus:border-blue-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md">
                            ...
                            <input type="text" name="group" id="group_new" value="{{ old('group', $setting->group) }}" 
                                   class="mt-1 focus:ring-blue-500 focus:border-blue-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md hidden"
                                   placeholder="Enter new group name">
                        </div>
                        <p class="mt-1 text-sm text-gray-500">Logical grouping for related settings</p>
                    </div>
                    ...
                    <div class="col-span-2">
                        ...
                        <textarea name="description" id="description" rows="3"
                                  class="mt-1 focus:ring-blue-500 focus:border-blue-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md"
                                  placeholder="Describe what this setting is used for">{{ old('description', $setting->description) }}</textarea>
                    </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const groupNew = document.getElementById('group_new');
            
            // Initialize state based on existing value
            if (!groupSelect.querySelector(`option[value="${groupNew.value}"]`)) {
                groupSelect.value = 'new';
                groupNew.classList.remove('hidden');
            } else {
                groupSelect.value = groupNew.value;
            }
            ...
            groupSelect.addEventListener('change', function() {
                if (this.value === 'new') {
                    groupNew.classList.remove('hidden');
                    groupNew.focus();
                    groupNew.required = true;
                } else {
                    groupNew.classList.add('hidden');
                    groupNew.required = false;
                    if (this.value !== '') {
                        groupNew.value = this.value;
                    }
                }
            });
        });
