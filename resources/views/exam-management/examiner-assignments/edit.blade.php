...
<div class="container mx-auto px-6 py-8">
    <div class="flex justify-between items-center mb-6">
        <h2 class="text-2xl font-semibold text-gray-700">Edit Examiner Assignment</h2>
    ...
    <div class="bg-white shadow-md rounded px-8 pt-6 pb-8 mb-4">
        <form action="{{ route('examiner-assignments.update', $assignment) }}" method="POST">
            @csrf
            @method('PUT')
            ...
            <div class="mb-4">
                ...
                <select class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline @error('user_id') border-red-500 @enderror"
                    ...
                    <option value="{{ $examiner->id }}" {{ old('user_id', $assignment->user_id) == $examiner->id ? 'selected' : '' }}>
                        {{ $examiner->name }}
                    </option>
                    @endforeach
                </select>
            ...
            <div class="mb-4">
                ...
                <select class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline @error('academic_session_id') border-red-500 @enderror"
                    ...
                    <option value="{{ $session->id }}" {{ old('academic_session_id', $assignment->academic_session_id) == $session->id ? 'selected' : '' }}>
                        {{ $session->name }}
                    </option>
                    @endforeach
                </select>
